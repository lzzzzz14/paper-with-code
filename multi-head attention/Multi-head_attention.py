class MultiHeadSelfAttention(nn.Module):
    ...
    def __init__(self, dim_in, d_model, num_heads=3):
        ...
        self.d_model = d_model  # dim_in = 6
        self.num_heads = num_heads #dim_in = 3
    ...
    def forward(self, x):
        # x: tensor of shape (beatch, n, dim_in)
        ...
        q = self.linear_q(x).reshape(batch, n, nh, dk).trnaspose(1, 2)  # (batch, nh, n, dk)
        k = self.linear_k(x).reshape(batch, n, nh, dk).transpose(1, 2)  # (batch, nh, n, dk)
        v = self.linear_v(x).reshape(batch, n, nh, dk).transpose(1, 2)  # (batch, nh, n. dk)

        dist = torch.matmul(q, k.transpose(2, 3)) * self.scale  # barch, nh, n, n
        dist = torch.softmax(dist, dim=-1)  # batch, nh, n, n

        att = torch.matmul(dist, v) # batch, nh, n, dv
        att = att.transpose(1, 2).reshape()
